...
# df_to_analyze = df_public  # if only public tweets
df_to_analyze = df_RBC      # if everything (official + public)
...
# Before LDA processing, clean the data by removing NaN values
df_to_analyze = df_to_analyze.dropna(subset=['text'])

# Then reset the index to ensure continuous indexing
...
# Prepare data
texts = df_to_analyze['text'].tolist()

# Convert to document-term matrix
vectorizer = CountVectorizer(max_df=0.95, min_df=2, stop_words='english',token_pattern=r'(?u)(?!https)\b\w\w+\b')
...
# Save the results
df_to_analyze.to_csv('rbc_lda_classified.csv', index=False)
